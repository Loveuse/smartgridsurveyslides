...
\textbf{Modello Matematico}
	\begin{block}{Vettore delle misurazioni} % vorrei mettere tipo l'alert
		...
		\begin{itemize}
			\item $\textbf{h(x)}$: relazione non lineare tra le misure $\textbf{z}$
			\item $\textbf{x}$: stato del sistema
...
\begin{frame}{Principali vulnerabilità delle Smart Grid: Jamming}
	...
	\pause
	\begin{block}{}
	Un consumer sceglie un tempo casuale per cambiare la propria power response evitando che l'attaccante possa predire il comportamento dell'utente e quindi capire in che modo varia il prezzo della corrente
